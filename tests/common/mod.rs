...
/// Run unscan with --audit on a rasterized specimen, then char-misses.py
/// for spatial ground-truth comparison against the vector PDF.
///
/// `raster` is the rasterized input PDF.
/// `vector` is the original vector PDF (ground truth).
/// `fontmap` is the font name → file path JSON.
/// `label` is used to name the audit directory (e.g. "300dpi-aa").
pub fn measure_accuracy(raster: &Path, vector: &Path, fontmap: &Path, label: &str) -> AccuracyResult {
    ...
    let (hits, misses, unmatched, skipped, total) = parse_accuracy_summary(&combined);
...
pub fn parse_accuracy_summary(output: &str) -> (usize, usize, usize, usize, usize) {
    let mut hits = 0;
    let mut misses = 0;
    let mut unmatched = 0;
    let mut skipped = 0;
