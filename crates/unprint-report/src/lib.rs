...
use unprint::audit::{AuditEntry, Decision};

use unprint::char_render;
use unprint::ground_truth::{self, GroundTruth};
use unprint::font_scan::FontEntry;
use unprint::glyph_map::NgramGlyphMap;
...
fn pick_interesting_observations<'a>(
    chars: &'a [unprint::audit::ObservationVote],
    n_show: usize,
    _n_normal: usize,
    ...
    if result.len() < n_show {
        ...
        let mut by_prob: Vec<(usize, &unprint::audit::ObservationVote)> =
                ...
                let kind = if e.decision == Decision::KeptRaster {
                    ...
                    if ps_match {
                        ...
                        let is_minor = match (picked_path, gt_path) {
                            (Some(pp), Some(gp)) => {
                                match (unprint::font_scan::read_font_identity(&pp),
                                       unprint::font_scan::read_font_identity(&gp)) {
...
fn render_correct_font_comparison(
    ...
    let words: Vec<unprint::ocr::TextRegion> = entry.word_bboxes.iter().map(|wb| {
        unprint::ocr::TextRegion {
            text: wb.text.clone(),
            x: wb.x, y: wb.y,
...
fn compute_inferred_font_size(
    font_data: &[u8],
    word_bboxes: &[unprint::audit::WordBBox],
    variant_tag: &str,
    ...
    let per_word: Vec<WordSizeDetail> = word_bboxes
        ...
        .filter_map(|wb| {
            let em_px = unprint::layout::width_matched_em_px_shaped(
                font_data,
                &wb.text,
                wb.width as f32,
                variant_tag,
...
fn build_observation_table(
    entry: &AuditEntry,
    obs_to_show: &[(usize, &unprint::audit::ObservationVote)],
    audit_root: &Path,
    correct_fe: Option<&FontEntry>,
...
pub fn generate_report(
    ...
    let sim_summary = {
        ...
        if sim_vals.is_empty() {
            ...
            let n = sim_vals.len();
            let above = sim_vals.iter().filter(|&&v| v >= MIN_VERIFY_SIMILARITY).count();
