...
fn ensure_specimen() {
    ...
    eprintln!("[t61] Generating specimen via gen-specimen.py ...");
...
fn measure_accuracy() -> AccuracyResult {
    ...
    // Use the no-AA raster — t55 creates this, but rasterize as fallback
    let raster = test_doc("font-timeline-specimen-rasterized-noaa-300dpi.pdf");
    if !raster.exists() {
        eprintln!("[t61] Rasterizing 300dpi no-AA ...");
        assert!(
            common::rasterize_pdf(&vector_src, &raster, 300, false),
